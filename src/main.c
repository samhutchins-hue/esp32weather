...
typedef enum {
    SYSTEM_STATE_WAITING,     // waiting for wifi/sntp
    SYSTEM_STATE_INITIALIZE,  // one-time mqtt setup
    SYSTEM_STATE_OPERATING,   // reading and publishing
    SYSTEM_STATE_DISCONNECTED // wifi lost, pause publishing
} system_state_t;
...
static esp_err_t init_bmp280(i2c_master_bus_handle_t bus_handle,
    ...
    if (ret != ESP_OK || *bmp280_dev_hdl == NULL) {
        ESP_LOGE(BMP280, "BMP280 init failed: %s, handle: %p",
                 esp_err_to_name(ret), *bmp280_dev_hdl);
...
static void read_bmp280_task(void *pvParameters) {
    ...
    xEventGroupWaitBits(wifi_event_group, WIFI_CONNECTED_BIT | TIME_SYNCED_BIT,
